 ...
 */
static int try_parse_end_of_central_directory(struct zip_archive *archive,
					      uint32_t offset)
{
	uint16_t comment_length, cd_records, cd_offset, cd_size;
	struct end_of_central_directory_record *eocd =
		check_access(archive, offset,
			     sizeof(struct end_of_central_directory_record));
	if (eocd == NULL ||
...
static struct local_file_header *
local_file_header_at_offset(struct zip_archive *archive, uint32_t offset)
{
...
static int get_entry_at_offset(struct zip_archive *archive, uint32_t offset,
			       struct zip_entry *out)
{
	struct local_file_header *lfh =
		local_file_header_at_offset(archive, offset);
...
static struct central_directory_file_header *
cd_file_header_at_offset(struct zip_archive *archive, uint32_t offset)
{
			   ...
			   struct zip_entry *out)
{
	...
	for (i = 0; i < archive->cd_records; ++i) {
		...
		cdfh_name_length =
			unaligned_uint16_read(cdfh->file_name_length);
		...
		if ((cdfh_flags & FLAG_ENCRYPTED) == 0 &&
		    ...
		    memcmp(file_name, archive->data + offset,
			   file_name_length) == 0) {
			return get_entry_at_offset(
				archive, unaligned_uint32_read(cdfh->offset),
				out);
		}
...
int zip_archive_find_entry_at_offset(struct zip_archive *archive,
				     uint32_t target, struct zip_entry *out)
{
	uint32_t i, offset = archive->cd_offset;
	for (i = 0; i < archive->cd_records; ++i) {
		struct central_directory_file_header *cdfh =
		...
		if ((cdfh_flags & FLAG_ENCRYPTED) == 0 &&
		    (cdfh_flags & FLAG_HAS_DATA_DESCRIPTOR) == 0) {
			if (get_entry_at_offset(
				    archive,
				    unaligned_uint32_read(cdfh->offset), out)) {
				return -1;
			}
